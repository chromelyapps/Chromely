    ...
    public class CefLoader
    {
        ...
        public string FindCefArchiveName(ChromelyPlatform platform, Architecture processArchitecture, CefBuildNumbers build)
        {
            ...
            using (var client = new WebClient())
            {
                ...
                {
                    if (binaryNamePattern1.Contains("95.0.4638.69"))
                    {
                        return $"cef_binary_95.7.14%2Bg9f72f35%2Bchromium-95.0.4638.69_{platformIdentifier}_minimal.tar.bz2";
                    }
                }

                if (!string.IsNullOrEmpty(binaryNamePattern2))
                {
                    if (binaryNamePattern2.Contains("95.0.4638.69"))
                    {
                        return $"cef_binary_95.7.14%2Bg9f72f35%2Bchromium-95.0.4638.69_{platformIdentifier}_minimal.tar.bz2";
                    }
                }
